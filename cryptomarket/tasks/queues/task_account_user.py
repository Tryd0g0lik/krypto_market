    ...
    while queue_keys.qsize() > 0:
        ...
        async with lock:
            ...
            try:
                ...
                person_manager.client = (
                    client if person.client is None else person.client
                )
                    ...
                    try:
                        while person.active:
                            ...
                            if (
                                ...
                                person.refresh_token = msg_data["result"][
                                    "refresh_token"
                                ]
                            ...
                            if (
                                ...
                                person.is_password = msg_data["result"][0][
                                    "is_password"
                                ]
                                person.system_name = msg_data["result"][0][
                                    "system_name"
                                ]
                                ...
                                try:
                                    ...
                                    if (
                                        re.search(
                                            regex_key_deribit_person, deribit_person_key
                                        )
                                        is None
                                    ):
                                        log.error("Key is not true. ERROR")
                                        return False
                                    serialize_ = {
                                        k: v
                                    ...
                                    async with context_redis_connection() as redis:
                                        ...
                                        if result_cache is None:
                                            # This we not found the key "deribit:person"
                                            data_for_cache = {
                                                deribit_person_key: serialize_
                                            }
                                        else:
                                            # This we found the key "deribit:person"
                                            data_for_cache: dict = json.loads(
                                                result_cache
                                            )
                                            check_unique_list = [
                                                k
                                                for k, v in data_for_cache.items()
                                                if k == deribit_person_key
                                            ]
                                            if check_unique_list[0] is not None:
                                                data_for_cache[deribit_person_key] = (
                                                    serialize_
                                                )
                                            else:
                                                data_for_cache.__setitem__(
                                                    deribit_person_key, serialize_
                                                )
