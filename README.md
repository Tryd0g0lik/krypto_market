...
- Запуская docker-compose арр само создаёт базу данных, проверят и добавляем (`DEBUG` укажите `Fasle`) shema "`crypto`".
- В зависимости от "`DEBUG`" таблизы из базы данных имеют своё уникальное имя.
