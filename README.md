...
<details closed>
<summary>Проанализировав предоставленные данные тестирования, можно дать следующую характеристику нагрузки и производительности (сделай клик):</summary>


## 📊 **Общая характеристика нагрузки**

### **1. Базовые метрики**
- **Всего запросов:** 8 586
- **Средняя пропускная способность:** ~14.13 запросов/сек
- **Длительность теста:** ~10 минут (607 секунд)
- **Ошибки:** 0% — **отличный показатель стабильности!**

## 📈 **Анализ по эндпоинтам**

### **Эндпоинт 1: `GET v1_currency` (простая выборка)**
| Метрика | Значение | Оценка |
|---------|----------|--------|
| **RPS** | 7.06 | Нормально |
| **Avg time** | 81 мс | 🟢 Отлично |
| **P95** | 129 мс | 🟢 Отлично |
| **P99** | 162 мс | 🟢 Отлично |

**Характеристика:** Эндпоинт работает быстро и стабильно. Даже в 1% самых медленных запросов укладывается в 162 мс — это очень хороший показатель для API.

### **Эндпоинт 2: `GET v1_currency_filter` (с фильтрацией)**
| Метрика | Значение | Оценка |
|---------|----------|--------|
| **RPS** | 7.05 | Нормально |
| **Avg time** | 369 мс | 🟡 Средне |
| **P95** | 833 мс | 🟡 Требует внимания |
| **P99** | 962 мс | 🟠接近 к критическому |

**Характеристика:** Эндпоинт с фильтрацией работает медленнее в 4-5 раз. Это ожидаемо, если фильтрация требует сложных вычислений или запросов к БД. Однако P99 близок к 1 секунде — это пограничное значение для пользовательского опыта.

## 📊 **Сравнительный анализ**

| Параметр | v1_currency | v1_currency_filter | Разница |
|----------|-------------|-------------------|---------|
| Avg time | 81 мс | 369 мс | ⬆️ 4.6x |
| P95 | 129 мс | 833 мс | ⬆️ 6.5x |
| P99 | 162 мс | 962 мс | ⬆️ 5.9x |

**Вывод:** Эндпоинт с фильтрацией масштабирует время ответа линейно относительно сложности, что нормально, но требует оптимизации.

## 🚦 **Оценка стабильности системы**

### **Положительные моменты:**
- ✅ **0% ошибок** — система справляется с нагрузкой без сбоев
- ✅ **Стабильность** — нет резких скачков времени ответа
- ✅ **Хорошая пропускная способность** — 14 rps для 1GB сервера достойно

### **Требует внимания:**
- ⚠️ **P99 для фильтрации** (962 мс) — близок к 1 секунде
- ⚠️ **Разброс времени** на фильтрации: от 116 мс до 1221 мс (10x разница)

## 🎯 **Рекомендации по оптимизации**

### **Для `v1_currency_filter`:**
1. **Индексы в БД:** проверить наличие индексов по фильтруемым полям
2. **Кэширование:** добавить Redis-кэш для частых фильтров
3. **Оптимизация запросов:** проверить количество JOIN-ов и сложность
4. **Пагинация:** если фильтр возвращает много данных

### **Для сервера (1GB RAM):**
Ваши текущие показатели хороши для 1GB сервера. При росте нагрузки:
- **300K запросов/день** — текущая мощность
- **500K+ запросов/день** — потребуется оптимизация или апгрейд

## 📋 **Итоговая характеристика**

> Система показывает **высокую стабильность** (0% ошибок) и **хорошую производительность** для базовых операций. Эндпоинт с фильтрацией работает медленнее, но в рамках допустимого для текущей инфраструктуры. При сохранении текущей нагрузки (14 rps) система прослужит долго без сбоев.

**Оценка нагрузки:** 🟢 Зеленая зона (система справляется с запасом)
</details>
